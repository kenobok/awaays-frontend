...
import { Link, Outlet, useParams, useSearchParams, useLocation, useNavigate } from 'react-router-dom'
...
const Groups = () => {
    ...
    useEffect(() => {
        const loadEditGroup = async () => {
            if (isEdit && groups && slug) {
                const editGroup = groups.find(grp => grp.slug === slug);
                if (editGroup) {
                    let file = null;
                    if (editGroup.image_url) {
                        try {
                            const mimeType = 'image/jpeg'; 
                            file = await urlToFile(editGroup.image_url, 'group-image.jpg', mimeType);
                        } catch (error) {
                            console.error("Failed to convert image URL to file:", error);
                        }
                    }

                    setFormData({name: editGroup.name, description: editGroup.description, image: file || null});
                    setInputFocus({ name: true, description: true });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEdit) {
                const editGroup = groups.find(grp => grp.slug === slug);
                const res = await API.patch(`/community/groups/${editGroup.id}/`, formData, {
                    headers: { "Content-Type": "multipart/form-data" }
                });
                navigate('/dashboard/forums-and-groups')
            } else {
                const res = await API.post(`/community/groups/`, formData, {
                    headers: { "Content-Type": "multipart/form-data" }
                });
            }
            toast.success(`Group ${isEdit ? 'Edited' : 'Created'} successfully`)
            refetch()
            setFormData({ name: "", description: "", image: null });
            setInputFocus({ name: false, description: false });
        ...
        <main className={`group-wrp space-x-10 flex max-[768px]:flex-col mt-[5.5rem] max-[941px]:mt-[4rem] py-10 max-[651px]:pb-2 ${location.pathname.includes(slug) ? 'display-flex-col' : ''}`}>
            ...
            <div className={`fixed w-full h-full top-0 left-0 bg-[rgba(0,0,0,.2)] overflow-hidden ${!createGroupForm ? 'hidden' : ''}`}>
                <form ref={groupFormRef} onSubmit={handleSubmit} className='absolute top-[50%] left-[50%] -translate-y-[50%] -translate-x-[50%] w-xl max-[577px]:w-[90%] bg-[var(--bg-color)] py-7 px-5 rounded-xl shadow-lg'>
                    ...
                    <h4 className='font-semibold text-[1.2rem] mb-5 text-center border-b border-gray-100'>{isEdit ? 'Edit Group' : 'Create Group'}</h4>
