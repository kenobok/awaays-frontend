...
const ItemsRequest = () => {
    ...
    useEffect(() => {
        ...
        requests.forEach(req => {
            ...
            const donorId = item?.donor
            
            const isMyItem = donorId === user.id;
            const isRequesterMe = requester.id === user.id;

            if (isMyItem && !isRequesterMe) {
                if (!grouped[item.id]) {
                    grouped[item.id] = { item, requests: [] };
                }
                grouped[item.id].requests.push(req);
            }
        });
    
        setItemRequests(Object.values(grouped));
    }, [requests, user]);


    return (
        <motion.section className={`user-requests flex-1 p-7 pb-20 max-[577px]:px-0 max-[577px]:py-5 ${location.pathname === '/dashboard/item-requests' ? '' : 'hidden'}`} initial={{ opacity: 0, x: -300 }}  animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <h2 className='text-xl font-bold text-center border-b border-gray-100 min-[577px]:mb-10'>Item Requests</h2>
            {isError ?
            ...
            :
                <div className='requests-data min-[577px]:border border-gray-300 p-10 px-8 rounded-2xl max-[577px]:px-0'>
                    ...
                    :
                        <div className={``}>
                            { itemRequests.length === 0 ? <p className="text-center">No requests available.</p> :
                            <>
                                ...
                                <div className="hidden max-[577px]:block px-0">
                                    {itemRequests.map((req, index) => (
