...
import { forums, groups } from '../../components/utils/UtilsData'
                            ...
                            <tbody className='relative'>
                                {groups.length>0 ? groups.map((group, index) => (
                                    <tr key={index} className="border-b border-gray-300">
                                        ...
                                        <td>{group.admin}</td>
                                        <td className='h-full my-auto'><Link to={`/community/groups/${group.slug}`} className='inline-block my-auto border-2 border-[var(--p-color)] text-[var(--p-color)] rounded-lg shadow-lg py-[8px] pt-[9px] px-3 cursor-pointer leading-[.9rem]'>View group</Link></td>
                                    </tr>
                                )) 
                                :
                                <div className='text-center absolute left-[50%] tranform -translate-x-[50%] translate-y-[100%]'>
                                    <p className='mb-3'>Create or Join a group</p>
                                    <Link to="/community/groups" className='text-[var(--p-color)] border-2 border-[var(--p-color)] px-3 py-2 rounded-lg shadow-lg hover:text-[1.1rem]'>Create/Join</Link>
                                </div>
                                }
                            </tbody>
                        </table>
                        ...
                        <div className="hidden max-[577px]:block">
                            {
                                ...
                                <div key={index} className="space-y-1 leading-[1.2rem] p-3 mb-5 border-b border-gray-200 rounded-lg shadow-sm">
                                    ...
                                    <Link to={`/community/groups/${group.slug}`} className='inline-block my-auto border-2 border-[var(--p-color)] text-[var(--p-color)] rounded-lg shadow-lg py-[8px] pt-[9px] px-3 cursor-pointer leading-[.7rem]'>View Group</Link>
                                </div>
                                ))
                                :
