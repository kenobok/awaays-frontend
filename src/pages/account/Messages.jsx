...
const Messages = () => {
    ...
    return (
        <motion.section className={`space-y-7 flex-1 p-7 pb-5 max-[941px]:px-5 max-[651px]:px-0 max-[651px]:pb-0`} initial={{ opacity: 0, x: -300 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <h2 className={`text-xl font-bold text-center border-b border-gray-100 ${location.pathname !== '/dashboard/messages' ? 'hidden' : ''}`}>Messages</h2>
            <div className='flex justify-evenly flex-wrap gap-5'>
                <div className={`message-list relative min-[651px]:border border-gray-300 rounded-xl p-4 py-3 min-[651px]:shadow-lg w-[18rem] max-[651px]:flex-1 max-[768px]:w-[15rem] min-[651px]:h-[78vh] overflow-x-hidden overflow-y-auto ${isMessageList ? '' : 'hide-list'}`}>
                    ...
                    {isLoading ? <div className='h-[100%] max-[651px]:h-[60vh]'><Loader1 /></div> :
                ...
                <div className={`message-outlet relative flex-1 min-[651px]:border border-gray-300 rounded-xl p-4 py-0 min-[651px]:pb-2 min-[651px]:shadow-lg ${isMessageList ? 'hide-outlet' : 'small-outlet'}`}>
                    <Outlet />
                </div>
            </div>
        </motion.section>
